...
app.post('/verify-authentication', async (req, res) => {
  try {
    ...
    return res.status(400).send({ error: 'System error' });
  }
});
...
if (ENABLE_HTTPS) {
  ...
  const port = 8000;
  expectedOrigin = ['http://localhost:3001', 'http://localhost:8000'];
